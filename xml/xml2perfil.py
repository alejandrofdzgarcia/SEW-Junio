...
def generate_altimetry_svg(xml_file, svg_file, output_dir):
    """
    Genera archivos SVG con la altimetría de las rutas en el archivo XML.
    
    Args:
        xml_file: Ruta al archivo XML
        svg_file: Ruta donde se guardará el archivo SVG principal
    ...
    svg_width = 800
    svg_height = 400
    padding = 50  # Espacio en los bordes
    
    # Colores para las diferentes rutas
    route_colors = ["#4285F4", "#DB4437", "#F4B400", "#0F9D58", "#9C27B0"]
    
    # Inicializar el contenido SVG
    svg_content = f'''<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" width="{svg_width}" height="{svg_height}">
    <rect width="100%" height="100%" fill="#f0f0f0"/>
    <text x="{svg_width/2}" y="30" font-family="Arial" font-size="20" text-anchor="middle" fill="#333">Altimetría de Rutas</text>
'''
    ...
    # Encontrar valores mínimos y máximos para escalar el gráfico general
    if all_points:
        min_distance = 0
        max_distance = max(p[0] for p in all_points)
        min_altitude = min(p[1] for p in all_points)
        max_altitude = max(p[1] for p in all_points)
        
        # Asegurar que haya un rango de altitud
        if max_altitude == min_altitude:
            min_altitude = max_altitude - 10
        
        # Añadir un margen al rango de altitud
        altitude_range = max_altitude - min_altitude
        min_altitude = max(0, min_altitude - altitude_range * 0.1)
        max_altitude = max_altitude + altitude_range * 0.1
...
'''
        ...
        # Dibujar cada ruta en el gráfico general
        for idx, route in enumerate(route_data):
            ...
            # Crear polilínea para la ruta
            polyline_points = []
            for distance, altitude in route["points"]:
                # Convertir a coordenadas SVG
                x = padding + (distance - min_distance) / (max_distance - min_distance) * (svg_width - 2 * padding)
                y = svg_height - padding - (altitude - min_altitude) / (max_altitude - min_altitude) * (svg_height - 2 * padding)
                polyline_points.append(f"{x},{y}")
            
            # Añadir puntos para cerrar el polígono
            first_x = padding + (route["points"][0][0] - min_distance) / (max_distance - min_distance) * (svg_width - 2 * padding)
            last_x = padding + (route["points"][-1][0] - min_distance) / (max_distance - min_distance) * (svg_width - 2 * padding)
            ...
            # Añadir el polígono para el área bajo la línea
            svg_content += f'''
    <polygon points="{' '.join(polygon_points)}" fill="{route["color"]}" fill-opacity="0.2" stroke="none"/>
'''
            
            # Añadir la polilínea para la ruta
            svg_content += f'''
    <polyline points="{' '.join(polyline_points)}" fill="none" stroke="{route["color"]}" stroke-width="3"/>
'''
            
            # Añadir círculos en los puntos
            for i, (distance, altitude) in enumerate(route["points"]):
                x = padding + (distance - min_distance) / (max_distance - min_distance) * (svg_width - 2 * padding)
                y = svg_height - padding - (altitude - min_altitude) / (max_altitude - min_altitude) * (svg_height - 2 * padding)
                svg_content += f'''
    <circle cx="{x}" cy="{y}" r="4" fill="{route["color"]}" stroke="#fff" stroke-width="1"/>
'''
            
            # Añadir nombre de la ruta a la leyenda
            legend_y = 60 + idx * 20
            svg_content += f'''
    <rect x="{svg_width - 280}" y="{legend_y}" width="15" height="15" fill="{route["color"]}"/>
    <text x="{svg_width - 260}" y="{legend_y + 12}" font-family="Arial" font-size="12" fill="#333">{route["name"]} ({int(route["points"][-1][0])}m)</text>
'''
            ...
            generate_individual_svg(route, individual_svg_file)
...
'''
    ...
    # Guardar el archivo SVG principal
    with open(svg_file, 'w', encoding='utf-8') as f:
        f.write(svg_content)
    
    print(f"Archivo SVG principal generado: {svg_file}")
...
def main():
    ...
    svg_file = os.path.join(current_dir, "altimetria.svg")
    ...
    # Generar el SVG
    generate_altimetry_svg(xml_file, svg_file, output_dir)
    
    print("\nProceso completado.")
    print("Se ha generado un archivo SVG general con todas las rutas: altimetria.svg")
    print("También se han generado archivos SVG individuales para cada ruta.")
